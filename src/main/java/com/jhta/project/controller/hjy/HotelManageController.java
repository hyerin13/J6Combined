...
public class HotelManageController {
	...
	@Autowired Accommodations_tempService_hjy accommodations_tempservice;
	@Autowired Room_info_tempService_hjy roomInfo_tempservice;
	@Autowired Period_tempService_hjy period_tempservice;
	@Autowired Additional_fee_tempService_hjy additional_fee_tempservice;
	@Autowired ServletContext sc;
	@GetMapping("hjy/hotelForm")
	public String hotelInsertForm(String type) {
		return "user/hjy/hotelManage/test";
	}
...
//		찐db에 저장하기
		...
		//객실세부정보
		Room_InfoVo roomInfoVo = roomInfo_tempservice.find(Integer.parseInt(aid));
		...
		//기간정보
		PeriodVo periodVo= period_tempservice.find(Integer.parseInt(aid));
		...
		System.out.println("찐: "+n+"개 추가");
		System.out.println("찐: "+n1+"개 추가");
		System.out.println("찐: "+n2+"개 추가");
		System.out.println("찐: "+n3+"개 추가");
...
//		}
	...
	public String hotelRequest(AccommodationsVo accommodationsVo, Additional_feeVo additional_feeVo, 
			PeriodVo periodVo, Room_InfoVo roomInfoVo,MultipartHttpServletRequest mtfRequest) {
//		 MultipartFile file1,MultipartFile rfilemain,MultipartFile rfileextra1,MultipartFile rfileextra2
		...
		String roomorgfilename = fileList.get(1).getOriginalFilename();//전송된 파일명
		...
		String roomexorgfilename1 = fileList.get(2).getOriginalFilename();//전송된 파일명
		...
		//추가이미지2
		String roomexorgfilename2 = fileList.get(3).getOriginalFilename();//전송된 파일명
		String roomexsavefilename2 = UUID.randomUUID()+"_"+roomexorgfilename2;//저장할 파일명(중복되지 않는 이름으로 만들기)
		try {
			//1. 파일 업로드하기
			InputStream is = fileList.get(3).getInputStream();
		...
		int n2= period_tempservice.insert(periodVo);
				
		//추가요금정보
		additional_feeVo.setRiid(riidseq);
		int n3 = additional_fee_tempservice.insert(additional_feeVo);
		
		System.out.println("temp: "+n+"개 추가");
		System.out.println("temp: "+n1+"개 추가");
		System.out.println("temp: "+n2+"개 추가");
		System.out.println("temp: "+n3+"개 추가");
...
//		sb.append("<img src=\"http://xxxxxxxxxx.co.kr/images/mail_title.jpg\" width=\"700px\" height=\"80px\" />");
		...
		sb.append("<p style=\"background-color:#f4f4f4; color:#754c24; border:1px solid #d7d7d7; padding:30px; text-align:center; font-size:18px; font-weight:bold; margin-bottom:50px;\">숙소 등록 요청</p>");
		sb.append("<p style=\"line-height:1.6em;\">숙소 정보</p>");
		sb.append("<div style=\"background-color:#DFCFBE border-top:1px solid #d7d7d7; border-bottom:1px solid #d7d7d7; padding:10px 10px 10px 50px; margin-bottom:50px;\">");
		...
		sb.append("</p>");
		sb.append("</div>");
		sb.append("<p style=\"line-height:1.6em;\">세부정보</p>");
		sb.append("<div style=\"background-color:#DFCFBE left center no-repeat; border-top:1px solid #d7d7d7; border-bottom:1px solid #d7d7d7; padding:10px 10px 10px 50px;\">");
		...
		sb.append("</p>");
		sb.append("<p style=\"font-size:11px; color:#636363;\">");
		sb.append("대표이미지: "+"<img src ="+roomInfoVo.getRimainimg()+">");
		sb.append("</p>");
		sb.append("<p style=\"font-size:11px; color:#636363;\">");
		sb.append("추가이미지1: "+"<img src ="+roomInfoVo.getRiextraimg1()+">");
		sb.append("</p>");
		sb.append("<p style=\"font-size:11px; color:#636363;\">");
		sb.append("추가이미지2: "+"<img src ="+roomInfoVo.getRiextraimg2()+">");
		sb.append("</p>");
		...
		sb.append("<a href='http://localhost:8090/project/hjy/ok?aid="+accommodationsVo.getAid()+"'>등록</a>");
		sb.append("<a href='http://localhost:8090/project/hjy/no?aid="+accommodationsVo.getAid()+"'>거부</a>");
		sb.append("</div>");
		sb.append("</div>");
		sb.append("</div>");
		sb.append("</body>");
