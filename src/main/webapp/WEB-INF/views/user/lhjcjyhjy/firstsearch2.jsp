	...
	*/
		...
		if(!sessionStorage.getItem('id') ){
			...
			if($("#locationamount").val()!="none"){
				...
				//리스트가 나올때				
					...
					list(templist,$("#count_min").val(),$("#count_max").val(),locationmarker.getPosition()[Object.keys(locationmarker.getPosition())[0]],locationmarker.getPosition()[Object.keys(locationmarker.getPosition())[2]],$("#locationamount").val(),$("#sort").val())
			 	}
			}else{
				if($("#gomap").html()=="돌아가기"){
					...
					list(templist,$("#count_min").val(),$("#count_max").val())
			 	}
			}
		}
	})
	...
	$("#forFree input").click(function() {
		...
		if($("#locationamount").val()!="none"){
		 	if($("#gomap").html()=="돌아가기"){
				...
				list(templist,$("#count_min").val(),$("#count_max").val(),locationmarker.getPosition()[Object.keys(locationmarker.getPosition())[0]],locationmarker.getPosition()[Object.keys(locationmarker.getPosition())[2]],$("#locationamount").val())
		 	}
		}else{
			if($("#gomap").html()=="돌아가기"){
				  gomapchange(templist)
	...
	//거리를 통한 검색의 select옵션이 변경되었을때 실행될 함수 -지영
 	$("#locationamount").change(function() {
		...
		list(templist,$("#count_min").val(),$("#count_max").val(),locationmarker.getPosition()[Object.keys(locationmarker.getPosition())[0]],locationmarker.getPosition()[Object.keys(locationmarker.getPosition())[2]],$(this).val());
    	}
	...
	$("#sort").change(function(){
		...
		let templist = new Array();
		if($("#locationamount").val()=="none"){
			//list(templist,null,null,null)
		}else{
			//makeCircle(templist,locationmarker.getPosition()[Object.keys(locationmarker.getPosition())[0]],locationmarker.getPosition()[Object.keys(locationmarker.getPosition())[2]],$("#locationamount").val());
		...
		checklist(templist)
  		if($("#locationamount").val()!="none"){
			...
			//리스트가 나올때				
				...
				list(templist,$("#count_min").val(),$("#count_max").val(),locationmarker.getPosition()[Object.keys(locationmarker.getPosition())[0]],locationmarker.getPosition()[Object.keys(locationmarker.getPosition())[2]],$("#locationamount").val(),minprice,maxprice)
		 	}
		}else{
			if($("#gomap").html()=="돌아가기"){
				gomapchange(templist)
...
function list(facilities,minprice,maxprice,xcoordi,ycoordi,distance,sort,restar,agrade){
	let changecnt=0;
   $.ajax({
		url:"${pageContext.request.contextPath }/hjy/firstsearchajax",
		traditional :true,
		data:{"searchHotel":"${aaddress }","checkin":"${rcheckin }","checkout":"${rcheckout }","countRoom":"${countRoom }","countPeople":"${rimaxper}",
			"facilities":facilities,"xcoordi":xcoordi,"ycoordi":ycoordi,"distance":distance,"minprice":minprice,"maxprice":maxprice},
		type:"post",
		dataType:"json",
		success:function(data){
			if(data.code=='success'){
				   ...
				   if(xcoordi){
					   ...
					   //db에서 불러온 좌표
						...
						if(isdistancein(refcoordi, dbcoordi)<=distance){
							...
							html+="<h5>"
							console.log(data.list[i].restar)
							if(data.list[i].restar==5){
								html+="★★★★★"
							}else if(data.list[i].restar==4){
								html+="★★★★☆"
							...
							html+="</h5>"
							console.log(data.list[i].amountsum)
							html+="<h4>"+data.list[i].amountsum+"원</h4>";
							html+="<button class='btn' onclick=\"";
							html+="location.href='${pageContext.request.contextPath }/user/kjy/room_info?AID="+data.list[i].aid;
							html+="&person="+${rimaxper}+"&roomnum="+${countRoom}+"&startday="+${rcheckin}+"&endday="+${rcheckout}
